...
// Theme editor feature (Theme page only)
...
export function initThemeEditor() {
  ...
  function markSelectedPreset(id) {
    ...
    const btn = document.querySelector(
      `.presetbtn[data-preset-id="${esc(id)}"]`
    );
  ...
  function saveTheme() {
    ...
    const nextLight = { ...(existing.light?.vars || {}), ...(state.draft.light || {}) };
    const nextDark = { ...(existing.dark?.vars || {}), ...(state.draft.dark || {}) };
  ...
  function resetTheme() {
    ...
    try { sessionStorage.removeItem("theme.lastPresetId"); } catch {}
